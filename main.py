...
class Model_Run(object):
    ...
    def __init__(self, arg):
        ...
        self.MuLAN = MuLAN(args, self.num_symbols, self.embedding_size,
                           self.symbol2vec, use_pretrain=self.use_pretrain, finetune=args.fine_tune)
        self.MuLAN.to(args.device)

        self.batch_nums = 0

        self.ignored_parameters = list(map(id, self.MuLAN.entity_encoder.symbol_emb.parameters()))
        self.base_params = filter(lambda p: id(p) not in self.ignored_parameters, self.MuLAN.parameters())

        self.parameters = filter(lambda p: p.requires_grad, self.MuLAN.parameters())
        ...
        self.selected_neighbors = self.load_selected_neighbors()

    def load_embed(self):
        symbol_id = {}
        rel2id = json.load(open(args.datapath + '/relation2ids'))
    ...
    def load_selected_neighbors(self):
        print("load selected neighbors")
        assert self.max_neighbor != 0
        r2e2neighbors = json.load(
            open(args.datapath + "/neighbors_top30_128_intrain.json")
        )
        selected_neighbors = {}
        for rel, e2neighbors in r2e2neighbors.items():
            assert rel != ""
            e2neighbors_array = {}
            for ent, neighbors in e2neighbors.items():
                # json key is always str, even if dumped form python dict
                ent = int(ent)
                pad_need = self.max_neighbor - len(neighbors)
                neighbors_array = np.array(neighbors, dtype=np.int32)
                assert pad_need >= 0
                neighbors_array = np.pad(
                    neighbors_array,
                    ((0, pad_need), (0, 0)),
                    constant_values=self.num_symbols,
                )
                # pad is self.symbol_num
                assert neighbors_array.shape[0] == self.max_neighbor
                e2neighbors_array[ent] = neighbors_array
            selected_neighbors[rel] = e2neighbors_array
        return selected_neighbors

    def get_meta(self, left, right, rel=None, add_noise_rel=None):
        '''
        ...
        right_connections = torch.LongTensor(np.stack([self.connections[_, :, :] for _ in right], axis=0)).to(
            args.device)
        right_degrees = torch.IntTensor([self.e1_degrees[_] for _ in right]).to(args.device)
        '''
        if rel is None:
            left_connections = torch.LongTensor(np.stack([self.connections[_, :, :] for _ in left], axis=0)).to(
                args.device)
            left_degrees = torch.IntTensor([self.e1_degrees[_] for _ in left]).to(args.device)
            right_connections = torch.LongTensor(np.stack([self.connections[_, :, :] for _ in right], axis=0)).to(
                args.device)
            right_degrees = torch.IntTensor([self.e1_degrees[_] for _ in right]).to(args.device)
        elif add_noise_rel is None:
            left_connections = torch.LongTensor(np.stack([self.selected_neighbors[rel][_] for _ in left], axis=0)).to(
                args.device)
            left_degrees = torch.IntTensor([self.e1_degrees[_] for _ in left]).to(args.device)
            right_connections = torch.LongTensor(np.stack([self.selected_neighbors[rel][_] for _ in right], axis=0)).to(
                args.device)
            right_degrees = torch.IntTensor([self.e1_degrees[_] for _ in right]).to(args.device)
        else:
            add_noise_selected_entity = [self.selected_neighbors[rel] for rel in add_noise_rel]
            add_noise_selected_neighbors = reduce(lambda x, y: {**x, **y}, add_noise_selected_entity)
            left_connections = torch.LongTensor(np.stack([add_noise_selected_neighbors[_] for _ in left], axis=0)).to(
                args.device)
            left_degrees = torch.IntTensor([self.e1_degrees[_] for _ in left]).to(args.device)
            right_connections = torch.LongTensor(np.stack([add_noise_selected_neighbors[_] for _ in right], axis=0)).to(
                args.device)
            right_degrees = torch.IntTensor([self.e1_degrees[_] for _ in right]).to(args.device)

        return (left_connections, left_degrees, right_connections, right_degrees)

    def get_eval_meta(self, left, right, rel):
        '''
        left_connections = torch.LongTensor(np.stack([self.connections[_, :, :] for _ in left], axis=0)).to(args.device)
        left_degrees = torch.IntTensor([self.e1_degrees[_] for _ in left]).to(args.device)
        right_connections = torch.LongTensor(np.stack([self.connections[_, :, :] for _ in right], axis=0)).to(
        ...
        right_degrees = torch.IntTensor([self.e1_degrees[_] for _ in right]).to(args.device)
        '''
        left_connection = np.expand_dims(self.selected_neighbors[rel][left[0]], 0)
        left_degrees = torch.IntTensor([self.e1_degrees[_] for _ in left]).to(args.device)
        right_connection = np.expand_dims(self.selected_neighbors[rel][right[0]], 0)
        right_degrees = torch.IntTensor([self.e1_degrees[_] for _ in right]).to(args.device)

        left.remove(left[0])
        right.remove(right[0])

        left_connections = np.stack([self.connections[_, :, :] for _ in left], axis=0)
        right_connections = np.stack([self.connections[_, :, :] for _ in right], axis=0)

        left_connections = torch.LongTensor(np.concatenate((left_connection, left_connections), axis=0)).to(
            args.device)
        right_connections = torch.LongTensor(np.concatenate((right_connection, right_connections), axis=0)).to(
            args.device)
    ...
    def save(self, path=None):
        if not path:
            path = self.save_path
        torch.save(self.MuLAN.state_dict(), path)

    def load(self, path=None):
        if path:
            self.MuLAN.load_state_dict(torch.load(path))
        else:
            self.MuLAN.load_state_dict(torch.load(self.save_path))
    ...
    def train(self, args):
        ...
        for data in train_generate(args.datapath, args.batch_size, args.few, self.symbol2id, self.ent2id,
                                   self.e1rel_e2, args.noise_rate):

            support, query, false, support_left, support_right, query_left, query_right, false_left, false_right, rel, add_noise_rel = data

            self.batch_nums += 1

            support_meta = self.get_meta(support_left, support_right, rel, add_noise_rel)
            query_meta = self.get_meta(query_left, query_right, rel)
    ...
    def eval(self, mode='dev'):
        ...
        with torch.no_grad():
            ...
            hits1 = []
            mrr = []
            task_pool = list(test_tasks.keys())
            ...
            for query_ in test_tasks.keys():
                ...
                support_meta = self.get_meta(support_left, support_right, query_, add_noise_task)
                ...
                for triple in test_tasks[query_][few:]:
                    ...
                    query_meta = self.get_eval_meta(query_left, query_right, query_)

                    scores, _, __ = self.MuLAN(support, support_meta,
                ...
                logging.info(
                    'Number of candidates: {}, number of test examples {}'.format(len(candidates), len(hits10_)))
